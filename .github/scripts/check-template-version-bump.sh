...
for template in "${templates[@]}"; do
  ...
  old_template_version="$(template_version_for "${old_manifest}" "${template}")"
  new_template_version="$(template_version_for galaxio-pack.yaml "${template}")"
  ...
  if [[ -z "${old_template_version}" ]]; then
    if [[ "${new_template_version}" != "${new_pack_version}" ]]; then
      echo "${template} is newly renderable; set its template version to pack version ${new_pack_version}" >&2
      exit 1
    fi
    continue
  fi
